...
file = snakemake.input.host_mapped_matrix

psql_script = snakemake.output.host_expression_script
psql_data = snakemake.output.host_expression_psql
...
def create_psql_script(df):
    ...
    del_table = 'DROP TABLE IF EXISTS "host_expression";\n\n'
    create_table = """
CREATE TABLE "host_expression" (
    ...
    """
    import_data = """
\\copy host_expression FROM '/sql/data_table.tsv' WITH (DELIMITER E'\\t', NULL '');
    """
...
def main():
    ...
    df = df.drop(columns=["host_name", "host_biotype", "is_expressed"])
    ...
    df = df[["host_id", "value", "sample_type"]]
